...
def get_params_from_csv_and_fold(opts):
    ...
    for x in range(batches):
       ...
       for i in range(start,end):
           ...
           output_name= "redetect_candidate_no_%03d_dm_%.2f_acc_%.2f"%(cand_ids[i],folding_packet['dm'],folding_packet['acc'])
           ...
           try:
               process = subprocess.Popen("prepfold -ncpus 1 -nsub 64 -mask %s -noxwin -topo -p %s -pd %s -dm %s %s -o %s"%(opts.mask_path,str(folding_packet['period']),str(folding_packet['pdot']),str(folding_packet['dm']),input_name,output_name),shell=True,cwd=output_path)
